  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Trigger asChild>
        <button
          type="button"
          className="relative inline-flex h-11 w-11 items-center justify-center rounded-full border border-border/80 bg-surface/80 text-foreground shadow-[0_12px_30px_-18px_rgba(0,0,0,0.6)] backdrop-blur-xl transition-colors hover:bg-accent/10 hidden lg:flex"
      ...
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay
          className={cn(
            "fixed inset-0 z-[60] bg-background/70 backdrop-blur-sm motion-reduce:transition-none hidden lg:flex",
        ...
        <DialogPrimitive.Content
          className={cn(
            "fixed inset-y-0 right-0 z-[70] flex w-[min(84vw,360px)] flex-col border-l border-border bg-surface shadow-[-24px_0_60px_-30px_rgba(0,0,0,0.65)] outline-none hidden lg:flex",
            "data-[state=open]:animate-in data-[state=closed]:animate-out",
